...
- **The claim and the hand-out** go through a durable outbox (`lib/outbox.ts`), not a bare `fetch`. Both are written to localStorage before they're sent and only leave the queue once the server accepts them (or 409s/400s), so answers survive a dead network, a closed tab, and a device that doesn't come back into signal until later. Retries fire on reconnect, on tab focus, every 15s, and via `sendBeacon` on page hide.
